<?php
    ...
    */
    class ParserStartPraca implements IParser
    {
        ...
        public function getDataList($receivedData) {
            ...
            foreach($people as $person) {
                ...
                if (isset($attrs[self::ATTRIBUTE_ID])) {
                    ...
                    LogManager::log(LOG_WARNING, 'missing id for startpraca record '.trim($this->iconv('UTF-8', 'ISO-8859-2', (string)$person->surname)));
                }
                ...
                foreach ($this->colsMapping as $localKey => $dbKeyData) {
                    ...
                    $row[$dbKey] = substr(trim($this->iconv('UTF-8', 'ISO-8859-2', (string)$person->{$localKey})), 0, $dbKeyMaxLength);
                }
                ...
                if (!empty($person->extras)) {
                    ...
                    if (!empty($person->extras->test)) {
                        ...
                        foreach ($tests as $test) {
                            ...
                            // changed from ints to strings
                            //$questionId = (string)$attrs[self::ATTRIBUTE_QUESTION];
                            
                            $questionValue = (string)$test;
                            $questionValue = trim($this->iconv('UTF-8', 'ISO-8859-2', $questionValue));
                            ...
                            if (isset($this->questionsMapping[$questionId])) {
                                ...
                                $infoType = $mapping[self::FIELD_QUES_INFO_TYPE]; //single info, like street, 
                                // or multiple, like drv license
        ...
        private function iconv($srcEncoding, $targetEncoding, $subject) {
            
            //return $subject;
            return iconv($srcEncoding, $targetEncoding, $subject);
        }
    }
