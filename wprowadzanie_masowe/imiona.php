...
<?php
	...
	if($plik=fopen($zmienna,"r"))
	{
		...
		while(! feof($plik))
		{
			$insert="insert into imiona values (nextval('imiona_id_seq'),'";
			$ciag=addslashes(trim(fgets($plik)));
			$insert=$insert.$ciag;
			$zapytanie = "select id from imiona where lower(nazwa) = lower('".$ciag."');";
			$done = pg_query($database, $zapytanie);
			$insert=$insert."');";
			if (pg_num_rows($done) == 0)
			{
				$wykonaj = pg_query ($database,$insert);
				echo "Wprowadzono imie: ".$ciag.".<br>";
			}
			else
			{
				echo "Imie ".$ciag." jest juz w slowniku.<br>";
			}
		}
		fclose($plik);
		//fclose($plik2);
