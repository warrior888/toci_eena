...
<?php
	...
	if($plik=fopen($zmienna,"r"))
	{
		...
		while(! feof($plik))
		{
			$insert="insert into bank values (nextval('bank_id_seq'),'";
			$ciag=addslashes(trim(fgets($plik)));
			$insert=$insert.$ciag;
			$insert=$insert."');";
			$zapytanie = "select id from bank where lower(nazwa) = lower('".$ciag."');";
			$done = pg_query($database, $zapytanie);
			if (pg_num_rows($done) == 0)
			{
				$wykonaj = pg_query ($database,$insert);
				echo "Wprowadzono bank: ".$ciag.".<br>";
			}
			else
			{
				echo "Bank ".$ciag." jest juz w slowniku.<br>";
			}
		}
		fclose($plik);
		//fclose($plik2);
