...
<?php
	require("../conf.php");
	$zmienna="../uploads/";
	$zmienna=$zmienna."miejscowosci.txt";
	//$zmienna2="uploads/";
	...
	if($plik=fopen($zmienna,"r"))
	{
		...
		while(! feof($plik))
		{
			$insert="insert into miejscowosc values (nextval('miejscowosc_id_seq'),'";
			$ciag=addslashes(trim(fgets($plik)));
			$insert=$insert.$ciag;
			$insert=$insert."');";
			$zapytanie = "select id from miejscowosc where lower(nazwa) = lower('".$ciag."');";
			$done = pg_query($database, $zapytanie);
			if (pg_num_rows($done) == 0)
			{
				$wykonaj = pg_query ($database,$insert);
				echo "Wprowadzono miejscowosc: ".$ciag.".<br>";
			}
			else
			{
				echo "Miejscowosc ".$ciag." jest juz w slowniku.<br>";
			}
		}
		fclose($plik);
		//fclose($plik2);
